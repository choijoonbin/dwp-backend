...
CREATE TABLE IF NOT EXISTS sys_domain_event_outbox (
    ...
    CONSTRAINT uq_domain_event_outbox_aggregate_sequence UNIQUE (
        event_source, aggregate_type, aggregate_id, aggregate_sequence)
);
...
CREATE INDEX IF NOT EXISTS idx_domain_event_outbox_aggregate
    ON sys_domain_event_outbox (
        event_source, aggregate_type, aggregate_id, aggregate_sequence, status);
...
CREATE TABLE IF NOT EXISTS sys_domain_event_offsets (
    ...
    last_event_id UUID,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (consumer_name, event_source, aggregate_type, aggregate_id)
);
