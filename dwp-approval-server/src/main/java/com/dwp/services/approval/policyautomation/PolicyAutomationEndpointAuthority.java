...
@Component
final class PolicyAutomationEndpointAuthority {
    static final String RESOURCE = "ADMIN.APPROVAL_POLICY";
    static final String UPDATE_CAPABILITY = "approvals.policy.update";
    static final String PUBLISH_CAPABILITY = "approvals.policy.publish";
    ...
    PolicyAutomationEndpointAuthority(
    ...
    Permit begin(
        ...
        if (!publish && !UPDATE_CAPABILITY.equals(capability)) {
            throw forbidden("The policy command capability is not supported.");
        }
        ...
        var matches = ApprovalPilotAuthorizationContext.current().orElseThrow(() -> unavailable(
                ...
                .toList();
        if (matches.size() != 1) {
            throw forbidden("The current route does not authorize this policy command.");
        }
    ...
    private Current current(Set<String> actions) {
        ...
        if (!routePresent || decision.validUntil() == null
            ...
            throw forbidden("Policy-automation scope or decision evidence does not match.");
        }
    ...
    private void requireHeaders(
        ...
        if (headers == null
            ...
            throw new BaseException(ErrorCode.STEP_UP_CHALLENGE_MISMATCH,
                    "Policy command evidence does not match the current object and decision.");
        }
    }
