...
@Component
final class IncidentEndpointAuthority {
    static final String RESOURCE = "ADMIN.APPROVAL_OPERATIONS";
    static final String EXECUTE_CAPABILITY = "approvals.operations.execute";
    ...
    Permit begin(
        ...
        Current current = current(Set.of("EXECUTE", "MANAGE"));
        var high = ApprovalPilotAuthorizationContext.current().orElseThrow(() -> unavailable(
                        "Current Approval route authority is unavailable.")).stream()
                .filter(value -> value.highRisk()
                        && current.decision().routeContractKey().equals(value.routeContractKey())
                        && EXECUTE_CAPABILITY.equals(value.capabilityContractKey()))
                .findFirst().orElseThrow(() -> forbidden(
                        "The current route does not authorize this incident command."));
        if (!"STEPUP-MGMT-HIGH-V1".equals(high.activationPolicy())) {
            throw forbidden("The current route does not authorize this incident command.");
        }
        requireHeaders(headers, current, expectedVersion);
        ApprovalStepUpVerifier.CommandBinding binding = new ApprovalStepUpVerifier.CommandBinding(
                ...
                high.activationPolicy(), EXECUTE_CAPABILITY, current.scope().opaqueScopeKey(),
    ...
    private Current current(Set<String> actions) {
        ...
        if (!routePresent || decision.validUntil() == null
            ...
            throw forbidden("Incident scope or decision evidence does not match.");
        }
    ...
    private void requireHeaders(
            ApprovalStepUpHeaders headers, Current current, long expectedVersion) {
        if (headers == null || headers.challenge() == null || headers.challenge().isBlank()
            ...
            throw new BaseException(ErrorCode.STEP_UP_CHALLENGE_MISMATCH,
                    "Incident command evidence does not match the current object and decision.");
        }
    }
