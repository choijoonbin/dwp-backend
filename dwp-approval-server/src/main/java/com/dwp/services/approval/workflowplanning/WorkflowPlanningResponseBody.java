...
import static com.dwp.services.approval.workflowplanning.WorkflowPlanningProtocol.BODY_MAX;
...
final class WorkflowPlanningResponseBody implements HttpResponse.BodySubscriber<byte[]> {
    ...
    synchronized void cancel() {result.completeExceptionally(new java.util.concurrent.CancellationException());if(subscription!=null) subscription.cancel();}
}
