...
@Component
public final class ApprovalTemplateAuthority {
    private final ApprovalIdentityDirectory identities;

    public ApprovalTemplateAuthority(ApprovalIdentityDirectory identities) { this.identities = identities; }
    ...
    public Access require(String... actions) {
        ...
        } catch (IllegalStateException exception) { throw unavailable(); }
        ...
        try { current = identities.require(actor.tenantId(), actor.userId()); }
        ...
        if (current == null || !current.active() || !actor.tenantId().equals(current.tenantId())
                ...
                || current.roles().stream().anyMatch(role -> role.startsWith("PROVIDER_"))) throw forbidden();
        ...
        return new Access(actor, scope, Set.copyOf(Arrays.asList(actions)));
    }
    ...
    private BaseException forbidden() { return new BaseException(ErrorCode.FORBIDDEN, "Current Approval template authority is required."); }
    private BaseException unavailable() { return new BaseException(ErrorCode.AUTHORITY_RESOLUTION_UNAVAILABLE,
            "Current Approval template authority cannot be revalidated."); }
}
