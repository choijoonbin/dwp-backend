# SynapseX API 명세서 (프론트엔드 전달용)

> **버전**: 1.0  
> **작성일**: 2026-02-03  
> **원본**: `services/synapsex-service/docs/SYNAPSEX_API_SPECIFICATION.md`  
...
> X-Tenant-ID 필수, X-Agent-ID 선택 (에이전트 감사 시)

### Read Tools
| Method | Path | 설명 |
|--------|------|------|
| GET | /agent-tools/cases/{caseId} | 케이스 상세 |
| GET | /agent-tools/documents | 전표 목록 (bukrs, gjahr, vendorId, customerId, fromDate, toDate, amountMin, amountMax, anomalyFlags) |
| GET | /agent-tools/documents/{bukrs}/{belnr}/{gjahr} | 전표 상세 (header+lines+reversal chain) |
| GET | /agent-tools/entities/{entityId} | Entity 360 |
| GET | /agent-tools/open-items | 오픈아이템 (type, overdueBucket) |
| GET | /agent-tools/lineage?caseId=... | Journey (raw→ingestion→scoring→case→action) |

### Write Tools
| Method | Path | 설명 |
|--------|------|------|
| POST | /agent-tools/actions/simulate | 시뮬레이션 (before/after preview, validation, SAP impact) |
| POST | /agent-tools/actions/propose | 승인 필요 여부 판정, HITL pending 생성 |
| POST | /agent-tools/actions/{actionId}/execute | 승인 완료된 action 실행 |
