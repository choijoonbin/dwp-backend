...
Status: Active external policy Gate register; not implementation or release-evidence SSOT

Last verified: 2026-08-27

Scope: DWP 고객 딜리버리 의사결정, 환경별 운영 준비 Gate, 외부 시스템 연동과
보안·개인정보·법무 승인

## 1. 관리 원칙

이 문서는 고객 또는 운영 환경의 결정이 필요한 외부 정책 Gate 등록부다. 구현·자동 시험·출시
증적의 실행 SSOT는 `dwp-frontend/docs/06-delivery`의 Delivery 문서와
`release-evidence/release-readiness.json`, `release-evidence/provider-tenant-acceptance.json`이다.
이 문서의 `IMPLEMENTED` 또는 `PARTIAL` 표기는 외부 Gate Adapter의 현재 기준선을 설명할 뿐,
PT 수용 항목이나 Production Release 완료를 선언하지 않는다. 과거 조사 문서의 표는 역사적
Snapshot이며 현재 Release 판정의 근거로 사용하지 않는다.
...
### 2.1 외부 정책 Gate 준비 상태

| 상태            | 의미                                                                    |
| --------------- | ----------------------------------------------------------------------- |
| `IMPLEMENTED`   | 외부 Gate를 기록·검증할 코드 기준선이 있다. Release 완료 의미는 아니다. |
...
| `G-11 TENANT_KMS`               | Tenant별 Customer-managed Key     | `PARTIAL`       | 내부 `KeyProvider`·Envelope v2·Local Guard 완료. 실제 Managed KMS Adapter, Workload Identity, Tenant CMK 선택, Rotation·Rewrap·삭제 보호·RTO/RPO 증거 필요 |
...
| 암호화      | 버전형 Local KEK + Per-write Envelope DEK | Managed KMS                    | Tenant CMK 또는 승인된 Tenant 격리 Key                   |
...
## 8. 외부 Gate Adapter Queue

다음 작업은 고객 값 없이도 계약과 Adapter 골격을 개발할 수 있다. 이 Queue는 Frontend
Release evidence 원장과 경쟁하는 구현 SSOT가 아니다. 실제 연결 성공 또는 운영 준비 완료
표시는 해당 Gate 증거가 승인되고 실행·증적 원장이 갱신될 때까지 금지한다.

| 우선순위 | 작업                                                                                                                   | 연결 Gate              |
| -------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `P0`     | 공통 `KeyProvider`·Envelope v2·환경 시작 Guard는 완료. Managed KMS Adapter와 Managed Identity Credential Provider 연결 | `G-01`, `G-11`         |
