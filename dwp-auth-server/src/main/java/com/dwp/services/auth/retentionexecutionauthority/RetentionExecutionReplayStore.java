...
public final class RetentionExecutionReplayStore {
    ...
    public RetentionExecutionReplayStore(StringRedisTemplate redis,Clock clock) {this.redis=redis;this.clock=clock;}
    ...
    public void consume(RetentionExecutionProofVerifier.Verified proof,Instant expiry) {
        if(redis==null) throw unavailable();if(expiry==null || expiry.isAfter(proof.expiresAt())) throw denied();
        ...
        String prefix="dwp:auth:retention-execution:{"+proof.bindings().target().tenantId()+':'+OWNER_PURPOSE+"}:";final Long result;
