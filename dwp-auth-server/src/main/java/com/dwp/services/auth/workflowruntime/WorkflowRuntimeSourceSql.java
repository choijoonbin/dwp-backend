...
final class WorkflowRuntimeSourceSql {
    private WorkflowRuntimeSourceSql() { }
    // Parameterized point lookups prevent fresh-database row estimates from repeatedly evaluating the UNION per tenant subject.
    static final String MEMBERSHIPS = """
            WITH requested_users AS MATERIALIZED (SELECT unnest(CAST(:users AS bigint[])) AS user_id), membership_sources AS (
              ...
              SELECT membership.user_id,assignment.role_id,assignment.valid_to,
                     jsonb_build_object('kind','GROUP','id',assignment.group_role_assignment_id,'version',assignment.version,
                         ...
                         'from',assignment.valid_from,'to',assignment.valid_to) AS source
              ...
              SELECT active_grant.user_id,active_grant.role_id,active_grant.expires_at,
                     jsonb_build_object('kind','PRIVILEGED','id',active_grant.active_privileged_grant_id,
                         'from',active_grant.activated_at,'to',active_grant.expires_at) AS source
               ...
               WHERE active_grant.tenant_id=:tenant AND active_grant.user_id IN(SELECT user_id FROM requested_users) AND active_grant.scope_type='TENANT'
                 AND active_grant.revoked_at IS NULL AND active_grant.activated_at<=CURRENT_TIMESTAMP AND active_grant.expires_at>CURRENT_TIMESTAMP
            )
            SELECT source.user_id,source.role_id,source.expiry,source.source::text,
                   role.code,role.version,role.updated_at
              FROM membership_sources source
              CROSS JOIN LATERAL (SELECT code,version,updated_at FROM com_roles active_role
                 WHERE active_role.tenant_id=:tenant AND active_role.role_id=source.role_id AND active_role.status='ACTIVE' OFFSET 0) role
              CROSS JOIN LATERAL (SELECT user_id FROM com_users current_subject
                 WHERE current_subject.tenant_id=:tenant AND current_subject.user_id=source.user_id
                   AND current_subject.status='ACTIVE' AND current_subject.identity_plane='TENANT' OFFSET 0) subject
    ...
    static final String ROLE_PERMISSIONS = """
            SELECT permission.role_id,resource.key || ':' || code.code AS permission_key,permission.effect,
             ...
             WHERE permission.tenant_id=:tenant AND permission.role_id=ANY(:roles)
               AND resource.key || ':' || code.code IN('APP.APPROVALS:VIEW','ACTION.APPROVAL_TASK:VIEW',
                   'ACTION.APPROVAL_TASK:APPROVE','ACTION.APPROVAL_REQUEST:UPDATE','ACTION.APPROVAL_FORM:VIEW')
    ...
    static final String PRINCIPAL_PERMISSIONS = """
            SELECT subject.user_id,resource.key || ':' || permission.code AS permission_key,grant_record.effect,grant_record.valid_to AS expiry,
                   jsonb_build_object('id',grant_record.principal_resource_grant_id,'version',grant_record.version,
                       'principalType',grant_record.principal_type,'principalRef',grant_record.principal_ref,
                       'effect',grant_record.effect,'from',grant_record.valid_from,'to',grant_record.valid_to,
                       'resource',resource.resource_id,'key',resource.key,'resourceUpdated',resource.updated_at,
                       'permission',permission.permission_id,'code',permission.code)::text AS source
             ...
             WHERE subject.tenant_id=:tenant AND subject.user_id=ANY(:users) AND subject.status='ACTIVE' AND subject.identity_plane='TENANT'
               AND grant_record.lifecycle_state='ACTIVE' AND grant_record.valid_from<=CURRENT_TIMESTAMP
               AND resource.key || ':' || permission.code IN('APP.APPROVALS:VIEW','ACTION.APPROVAL_TASK:VIEW',
                   'ACTION.APPROVAL_TASK:APPROVE','ACTION.APPROVAL_REQUEST:UPDATE','ACTION.APPROVAL_FORM:VIEW')
