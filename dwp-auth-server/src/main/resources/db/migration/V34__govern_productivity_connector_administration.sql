...
SELECT tenant_id,
       'ADMIN',
       'ADMIN.PRODUCTIVITY_CONNECTOR',
       'Productivity connector control plane',
       TRUE,
       1,
       1
  FROM com_tenants
...
SELECT role.tenant_id,
  ...
  JOIN com_resources resource
   ...
   AND resource.key = 'ADMIN.PRODUCTIVITY_CONNECTOR'
...
-- Force new permission snapshots for affected active administrator sessions.
   ...
   AND EXISTS (
       SELECT 1
        ...
        WHERE membership.tenant_id = session.tenant_id
          ...
          AND role.code IN ('ADMIN', 'TENANT_ADMIN', 'PLATFORM_ADMIN'));
