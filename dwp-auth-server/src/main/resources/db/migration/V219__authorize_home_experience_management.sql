-- Home Studio changes tenant-wide mode, composition, content, and release state.
-- Keep its authority separate from template and widget-policy administration.
INSERT INTO sys_tenant_resource_templates (
    resource_key, resource_type, display_name, required_entitlement)
VALUES (
    'ADMIN.HOME_EXPERIENCE', 'ADMIN', 'Home experience', 'core.workspace')
...
VALUES
    ('TENANT_ADMIN', 'ADMIN.HOME_EXPERIENCE', 'VIEW', 'ACTIVE'),
    ('TENANT_ADMIN', 'ADMIN.HOME_EXPERIENCE', 'MANAGE', 'ACTIVE')
...
SELECT tenant.tenant_id, template.resource_type, template.resource_key,
 ...
 WHERE template.resource_key = 'ADMIN.HOME_EXPERIENCE'
...
-- Durable session projections must observe the new authority immediately.
