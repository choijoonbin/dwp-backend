-- Evaluation evidence export is intentionally separate from VIEW so a reviewer
-- cannot extract tenant quality evidence unless the delegated role permits it.
INSERT INTO sys_tenant_role_permission_templates (
    role_code, resource_key, permission_code)
VALUES
    ('DWAION_ADMIN', 'ADMIN.DWAION_EVALUATION', 'EXPORT'),
    ('DWAION_EVALUATOR', 'ADMIN.DWAION_EVALUATION', 'EXPORT')
...
SELECT role.tenant_id, role.role_id, resource.resource_id,
 ...
 WHERE template.lifecycle_state = 'ACTIVE'
   AND template.role_code IN ('DWAION_ADMIN', 'DWAION_EVALUATOR')
   AND template.resource_key = 'ADMIN.DWAION_EVALUATION'
   AND template.permission_code = 'EXPORT'
 ...
 WHERE EXISTS (
       SELECT 1
        ...
        WHERE membership.tenant_id = user_record.tenant_id
          ...
          AND role.code IN ('DWAION_ADMIN', 'DWAION_EVALUATOR'));
