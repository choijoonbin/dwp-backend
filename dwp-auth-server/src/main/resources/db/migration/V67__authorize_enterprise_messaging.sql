-- Enterprise Messaging separates daily app usage from tenant policy
-- administration. Messaging administrators manage policy and operations
-- metadata; private conversation membership remains the body access boundary.
INSERT INTO sys_builtin_role_catalog (
    ...
    assignment_class)
VALUES (
    'MESSAGING_ADMIN', 'Messaging administrator',
    'Maintains tenant messaging policy, retention, Space-linked messaging posture, and operational readiness without reading private conversations.',
    'WORKSPACE', '{"ko":"메신저 관리자","en":"Messaging administrator"}',
    TRUE, TRUE, 58, 'ACTIVE', 'DELEGATED')
...
SELECT grantor_role_code, 'MESSAGING_ADMIN', 'APPROVAL', 'ACTIVE'
...
VALUES ('AUDITOR', 'MESSAGING_ADMIN', 'AUDIT_INDEPENDENCE', 'ACTIVE', 'DENY', 'HIGH')
...
VALUES
    ('APP.MESSAGING', 'APP', 'Messenger', 'core.workspace'),
    ('ADMIN.MESSAGING', 'ADMIN', 'Messaging administration', 'core.workspace')
...
VALUES
    ('WORKSPACE_MEMBER', 'APP.MESSAGING', 'VIEW'),
    ('WORKSPACE_MEMBER', 'APP.MESSAGING', 'CREATE'),
    ('WORKSPACE_MEMBER', 'APP.MESSAGING', 'UPDATE'),
    ('TENANT_ADMIN', 'APP.MESSAGING', 'VIEW'),
    ('TENANT_ADMIN', 'ADMIN.MESSAGING', 'VIEW'),
    ('TENANT_ADMIN', 'ADMIN.MESSAGING', 'MANAGE'),
    ('MESSAGING_ADMIN', 'APP.MESSAGING', 'VIEW'),
    ('MESSAGING_ADMIN', 'ADMIN.MESSAGING', 'VIEW'),
    ('MESSAGING_ADMIN', 'ADMIN.MESSAGING', 'UPDATE'),
    ('MESSAGING_ADMIN', 'ADMIN.MESSAGING', 'MANAGE')
...
SELECT tenant.tenant_id, catalog.role_code, catalog.display_name,
  ...
  JOIN sys_builtin_role_catalog catalog ON catalog.role_code = 'MESSAGING_ADMIN'
...
SELECT tenant.tenant_id, template.resource_type, template.resource_key,
 ...
 WHERE template.resource_key IN ('APP.MESSAGING', 'ADMIN.MESSAGING')
...
SELECT role.tenant_id, role.role_id, resource.resource_id,
 ...
 WHERE template.lifecycle_state = 'ACTIVE'
   AND template.resource_key IN ('APP.MESSAGING', 'ADMIN.MESSAGING')
...
SELECT md5('app-resource-set:' || resource.tenant_id || ':' || resource.key)::uuid,
       resource.tenant_id, 'APP_MESSAGING', resource.name,
       'Administrative boundary for ' || resource.name,
       'APP', 'ACTIVE', 1, 1
  FROM com_resources resource
 WHERE resource.type = 'APP' AND resource.key = 'APP.MESSAGING'
...
SELECT md5('app-resource-member:' || resource.tenant_id || ':' || resource.key)::uuid,
  ...
  JOIN com_admin_resource_sets resource_set
   ...
   AND resource_set.resource_set_key = 'APP_MESSAGING'
 WHERE resource.type = 'APP' AND resource.key = 'APP.MESSAGING'
...
SELECT tenant_id, 'SKAX_MESSAGING_ADMINS', '메신저 운영 관리자',
       '메신저 정책, 보존, Space 연동과 운영 상태를 관리하는 기능 관리자 그룹',
       'LOCAL', 'seed:skax_messaging_admins', 'ACTIVE', 1, 1
  FROM com_tenants
...
SELECT access_group.tenant_id, access_group.group_id, user_record.user_id,
  ...
  JOIN com_users user_record
   ...
   AND user_record.email_normalized IN ('yujin.choi@sk.com', 'hyunwoo.park@sk.com')
 WHERE access_group.group_key = 'SKAX_MESSAGING_ADMINS'
...
SELECT access_group.tenant_id, access_group.group_id, role.role_id,
       ...
       'SKAX 메신저 운영 책임을 기능 관리자 그룹 단위로 위임합니다.', 1, 1
  FROM com_groups access_group
  JOIN com_roles role
    ON role.tenant_id = access_group.tenant_id
   AND role.code = 'MESSAGING_ADMIN'
 WHERE access_group.group_key = 'SKAX_MESSAGING_ADMINS'
   AND NOT EXISTS (
       SELECT 1
