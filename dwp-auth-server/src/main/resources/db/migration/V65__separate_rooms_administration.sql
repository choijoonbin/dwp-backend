-- Administration is intentionally distinct from APP.ROOMS so booking users do
-- not inherit room inventory, policy, or approval authority.
INSERT INTO sys_tenant_resource_templates (
    resource_key, resource_type, display_name, required_entitlement)
VALUES ('ADMIN.ROOMS', 'ADMIN', 'Meeting room administration', 'core.workspace')
...
VALUES
    ('TENANT_ADMIN', 'ADMIN.ROOMS', 'VIEW'),
    ('CALENDAR_ADMIN', 'ADMIN.ROOMS', 'VIEW'),
    ('CALENDAR_ADMIN', 'ADMIN.ROOMS', 'CREATE'),
    ('CALENDAR_ADMIN', 'ADMIN.ROOMS', 'UPDATE'),
    ('CALENDAR_ADMIN', 'ADMIN.ROOMS', 'MANAGE')
...
SELECT tenant.tenant_id, template.resource_type, template.resource_key,
  ...
  JOIN sys_tenant_resource_templates template
    ON template.resource_key = 'ADMIN.ROOMS'
...
SELECT role.tenant_id, role.role_id, resource.resource_id,
 ...
 WHERE template.lifecycle_state = 'ACTIVE'
   AND template.resource_key = 'ADMIN.ROOMS'
