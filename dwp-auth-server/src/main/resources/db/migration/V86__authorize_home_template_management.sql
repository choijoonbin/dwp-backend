-- Flow Home templates are tenant-governed workspace content. Provision the
-- resource for every tenant and grant bootstrap administration explicitly;
-- end-user personalization remains controlled by the platform tenant policy.
INSERT INTO sys_tenant_resource_templates (
    resource_key, resource_type, display_name, required_entitlement)
VALUES (
    'ADMIN.HOME_TEMPLATE', 'ADMIN', 'Home templates', 'core.workspace')
...
VALUES
    ('TENANT_ADMIN', 'ADMIN.HOME_TEMPLATE', 'VIEW', 'ACTIVE'),
    ('TENANT_ADMIN', 'ADMIN.HOME_TEMPLATE', 'MANAGE', 'ACTIVE')
...
SELECT tenant.tenant_id, template.resource_type, template.resource_key,
 ...
 WHERE template.resource_key = 'ADMIN.HOME_TEMPLATE'
...
-- Existing tokens that can carry the newly granted authority must be rotated.
