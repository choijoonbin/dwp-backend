-- Deep Research remains subordinate to APP.ASK and is tenant/self scoped by
-- the Agent service. Materialize the dedicated application authority for
-- existing and future workspace members so the runtime's exact VIEW/MANAGE
-- checks are grantable without broadening any administrative role.
INSERT INTO sys_tenant_resource_templates (
    resource_key, resource_type, display_name, required_entitlement)
VALUES
    ('APP.DWAION_RESEARCH', 'APP', 'DWAI-ON Deep Research', 'ai.agent-runtime')
...
VALUES
    ('WORKSPACE_MEMBER', 'APP.DWAION_RESEARCH', 'VIEW'),
    ('WORKSPACE_MEMBER', 'APP.DWAION_RESEARCH', 'MANAGE')
...
SELECT tenant.tenant_id,
 ...
 WHERE template.resource_key = 'APP.DWAION_RESEARCH'
...
SELECT role.tenant_id,
 ...
 WHERE template.lifecycle_state = 'ACTIVE'
   ...
   AND template.resource_key = 'APP.DWAION_RESEARCH'
