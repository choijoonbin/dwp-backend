...
/** Production Replay filter/controller, default Tomcat connector, no header/body limit overrides. */
public final class InformationReplayEmbeddedServer implements AutoCloseable {
    private final AnnotationConfigServletWebServerApplicationContext context;
    public InformationReplayEmbeddedServer(InformationReplayAuthorityService actual, boolean enabled) {
        context = new AnnotationConfigServletWebServerApplicationContext();
        try {
            context.getEnvironment().getPropertySources().addFirst(new MapPropertySource("replay-http-gate",
                    Map.of("dwp.auth.approval-information-replay.enabled", enabled)));
            ...
            context.registerBean(InformationReplayAuthorityService.class, () -> actual);
            context.registerBean(InformationReplayController.class, () -> new InformationReplayController(actual));
    ...
    public URI endpoint() { return URI.create("http://127.0.0.1:" + context.getWebServer().getPort() + InformationReplayProtocol.PATH); }
    @Override public void close() { context.close(); }
    @Configuration(proxyBeanMethods = false) @EnableWebMvc @EnableWebSecurity @Import(InformationReplaySecurityConfig.class)
    static class Web {
    ...
    @RestControllerAdvice static class Errors {
        @ExceptionHandler(BaseException.class) ResponseEntity<Void> reject(BaseException error) { return ResponseEntity.status(error.getErrorCode().getHttpStatus()).build(); }
    }
}
