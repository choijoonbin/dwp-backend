...
class CodeUsageServiceTest {
    ...
    void getCodeUsages_TenantIsolation() {
        ...
        when(codeUsageRepository.findByTenantIdAndFilters(eq(tenantId), any(), any(), any()))
                .thenReturn(page);
        
        // When
        com.dwp.services.auth.dto.admin.PageResponse<CodeUsageSummary> response = 
                codeUsageService.getCodeUsages(tenantId, 1, 20, null, null);
        
        // Then
        assertNotNull(response);
        assertEquals(1, response.getItems().size());
        assertEquals(tenantId, response.getItems().get(0).getTenantId());
        verify(codeUsageRepository).findByTenantIdAndFilters(eq(tenantId), any(), any(), any());
    }
}
