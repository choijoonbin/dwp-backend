...
class HomeWidgetPolicyAuthorizationPostgresTest {
    ...
    @Test
    void grantsEveryWidgetPolicyAuthorityToExistingTenantAdministrators() {
        PGSimpleDataSource dataSource = dataSource();
        migrate(dataSource, "214");
        JdbcTemplate jdbc = new JdbcTemplate(dataSource);
        Long tenantId = jdbc.queryForObject(
                "SELECT tenant_id FROM com_roles WHERE code = 'TENANT_ADMIN' ORDER BY tenant_id LIMIT 1",
                Long.class);
        ...
        Long userId = jdbc.queryForObject("""
                ...
                VALUES (?, 'Widget policy administrator',
                        'widget-policy-admin@test.invalid', 'ACTIVE')
                RETURNING user_id
                """, Long.class, tenantId);
        jdbc.update("""
        ...
        assertThat(jdbc.queryForList("""
                SELECT permission.code
                 ...
                 WHERE grant_record.tenant_id = ?
                   ...
                   AND resource.key = 'ADMIN.HOME_WIDGET_POLICY'
                 ORDER BY permission.code
                """, String.class, tenantId, roleId))
                .containsExactly("AUDIT", "EXPLAIN", "MANAGE", "PUBLISH", "VIEW");
        assertThat(jdbc.queryForObject("""
                SELECT required_entitlement
                  FROM sys_tenant_resource_templates
                 WHERE resource_key = 'ADMIN.HOME_WIDGET_POLICY'
