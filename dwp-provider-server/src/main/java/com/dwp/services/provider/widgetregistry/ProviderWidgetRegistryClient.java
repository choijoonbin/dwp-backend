...
@Component
public class ProviderWidgetRegistryClient {
    static final String OWNER_SCOPE_HEADER = "X-DWP-Widget-Owner-Product-Keys";
    static final String WIDGET_REGISTRY_TOKEN_HEADER = "X-DWP-Widget-Registry-Token";
    ...
    ProviderWidgetRegistryClient(
            ...
            @Value("${dwp.provider.widget-registry-platform-token:}") String platformServiceToken) {
    ...
    @Bulkhead(name = "platformWidgetRegistry", type = Bulkhead.Type.SEMAPHORE)
    @CircuitBreaker(name = "platformWidgetRegistry")
    public ResponseEntity<byte[]> forward(HttpServletRequest request, byte[] body) {
        ...
        URI target = UriComponentsBuilder.fromPath(internalPath(request.getRequestURI()))
        ...
        var outbound = platform.method(HttpMethod.valueOf(request.getMethod()))
                ...
                .headers(headers -> {
                    ...
                    headers.set(WIDGET_REGISTRY_TOKEN_HEADER, platformServiceToken);
    ...
    private static String internalPath(String path) {
        return switch (path) {
            case "/v1/admin/widget-definitions" -> INTERNAL_PREFIX + "/definitions";
            case "/v1/admin/widget-runtime-controls" -> INTERNAL_PREFIX + "/runtime-controls";
            default -> {
                if (path.startsWith("/v1/admin/widget-definitions/")) {
                    yield INTERNAL_PREFIX + "/definitions/"
                            + path.substring("/v1/admin/widget-definitions/".length());
                }
                if (path.startsWith("/v1/admin/widget-definition-versions/")) {
                    yield INTERNAL_PREFIX + "/definition-versions/"
                            + path.substring("/v1/admin/widget-definition-versions/".length());
                }
                if (path.startsWith("/v1/admin/widget-runtime-controls/")) {
                    yield INTERNAL_PREFIX + "/runtime-controls/"
                            + path.substring("/v1/admin/widget-runtime-controls/".length());
                }
                if (path.startsWith("/v1/admin/widget-registry/")) {
                    yield INTERNAL_PREFIX + "/registry/"
                            + path.substring("/v1/admin/widget-registry/".length());
                }
                throw new BaseException(
                        ErrorCode.INVALID_INPUT_VALUE,
                        "A canonical Provider Widget Registry path is required.");
            }
        };
    }
