...
REWRITE_COMMAND = "python3 scripts/check-source-size.py --write"
...
def governed_source_files() -> list[Path]:
    return sorted(ROOT.glob("dwp-*/src/main/java/**/*.java"))
...
def load_baseline() -> dict[str, int]:
    ...
    for relative, limit in value.items():
        if not valid_baseline_path(relative):
            raise ValueError(f"invalid governed source path {relative!r}")
        if (
...
def measured_sources() -> dict[str, int]:
    return {
        str(source_file.relative_to(ROOT)): line_count(source_file)
        for source_file in governed_source_files()
    }
...
def write_baseline(baseline: dict[str, int], measured: dict[str, int]) -> int:
    ...
    if increases:
        print(
            "Refusing to raise the source-size baseline; reduce these files first.",
            file=sys.stderr,
        )
    ...
    print(
        f"Updated source-size baseline with {len(exact_baseline)} exact exceptions."
    )
    return 0
...
def check_baseline(baseline: dict[str, int], measured: dict[str, int]) -> int:
    ...
    for relative in baseline:
        if relative not in measured:
            violations.append(
                f"{relative}: stale source-size baseline entry; run {REWRITE_COMMAND}"
            )
    ...
    print(
        f"PASS source-size budget: {len(measured)} production files checked; "
        f"exceptions are exact and new files are limited to {DEFAULT_LIMIT} lines."
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"Invalid source-size baseline: {exception}", file=sys.stderr)
        return 1
