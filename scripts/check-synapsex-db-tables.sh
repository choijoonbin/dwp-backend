...
if [ "$USE_DOCKER" = true ]; then
    ...
    if echo "$CONN_ERR" | grep -q "FATAL\|error\|could not connect"; then
        echo -e "  ${RED}접속 실패:${NC}"
        echo "$CONN_ERR" | sed 's/^/  /'
        echo -e "  ${YELLOW}→ SynapseX 서버와 동일한 DB를 쓰려면 동일한 Host/Port/User/DB를 사용해야 합니다.${NC}"
...
if [ -z "$SCHEMA_EXISTS" ] || [ "$SCHEMA_EXISTS" != "1" ]; then
    ...
    echo -e "  → SynapseX 서버를 한 번 기동하면 Flyway가 V1/V2에서 스키마를 생성합니다."
else
    echo -e "  ${GREEN}스키마 '${SCHEMA_NAME}' 존재${NC}"
fi
echo ""

# 2. Flyway 히스토리 (SynapseX는 schemas: dwp_aura 이므로 테이블이 dwp_aura 스키마에 있음)
...
if [ -z "$HAS_FLYWAY" ] || [ "$HAS_FLYWAY" != "1" ]; then
    ...
    echo -e "  → SynapseX 서버 기동 시 Flyway가 V1~V4 마이그레이션을 실행합니다."
else
...
if [ -z "$TABLES" ]; then
    ...
    echo -e "  → SynapseX 서버 기동 후 Flyway가 V3/V4에서 테이블을 생성합니다."
else
...
echo -e "${BLUE}참고: SynapseX 서버 기동 로그에서 'Flyway', 'Migrating', 'Successfully applied' 검색해 보세요.${NC}"
