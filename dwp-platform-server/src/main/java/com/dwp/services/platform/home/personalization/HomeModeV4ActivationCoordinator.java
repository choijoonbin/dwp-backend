...
public class HomeModeV4ActivationCoordinator implements ApplicationRunner {
    ...
    private void activateContinuityProjection() {
        ...
        jdbcTemplate.update("""
                ...
                SELECT gen_random_uuid(), classic.tenant_id, classic.user_id, classic.surface_key,
                       ...
                       CASE WHEN classic.is_default AND NOT EXISTS (
                           SELECT 1 FROM usr_home_views existing_default
                            WHERE existing_default.tenant_id = classic.tenant_id
                              ...
                              AND existing_default.mode_key = 'FLOW_V1'
                 ...
                 WHERE classic.mode_key = 'CLASSIC'
                   ...
                   AND upper(COALESCE(
                       experience.composition_policy ->> 'experienceVariant', 'CLASSIC')) = 'FLOW_V1'
                   AND NOT EXISTS (
                       SELECT 1 FROM usr_home_views flow
                        WHERE flow.tenant_id = classic.tenant_id
                          AND flow.user_id = classic.user_id
                          AND flow.surface_key = classic.surface_key
                          AND flow.mode_key = 'FLOW_V1'
                          AND flow.view_key = classic.view_key
        ...
        // preserved Classic rollback row after the Flow projection becomes authoritative.
        jdbcTemplate.update("""
