...
/** Bounds an internal Messaging response even when it arrives with chunked transfer encoding. */
final class MessagingSourceBodySubscriber implements HttpResponse.BodySubscriber<byte[]> {
        ...
        for (ByteBuffer buffer : buffers) {
            ...
            if (length > MAX_BYTES - output.size()) {
                ...
                result.completeExceptionally(
                        new IllegalStateException("Messaging source response exceeds the size limit."));
                return;
            }
