...
final class MailAccessSql {
    ...
    static final String ACCOUNT_SEND_ACCESS = """
             AND (
                 (account.account_kind = 'PERSONAL' AND account.owner_user_id = ?)
                 OR (
                     account.account_kind = 'SHARED'
    ...
    static final String THREAD_SEND_ACCESS = """
             AND account.tenant_id = thread.tenant_id
             AND account.account_id = thread.account_id
             AND (
                 (account.account_kind = 'PERSONAL' AND account.owner_user_id = ?)
    ...
    static final String THREAD_MANAGE_ACCESS = """
             AND account.tenant_id = thread.tenant_id
             AND account.account_id = thread.account_id
             AND (
                 (account.account_kind = 'PERSONAL' AND account.owner_user_id = ?)
                     ...
                     AND EXISTS (
                         SELECT 1
                           ...
                           JOIN mail_shared_inbox_access_grants access_grant
                            ...
                            AND access_grant.member_state = 'ACTIVE'
                            AND access_grant.can_read = TRUE
                            AND access_grant.can_manage = TRUE
                            AND (access_grant.expires_at IS NULL
                                 OR access_grant.expires_at > CURRENT_TIMESTAMP)
                          WHERE policy.tenant_id = thread.tenant_id
                            AND policy.allow_shared_inboxes = TRUE
    ...
    static final String THREAD_ASSIGN_ACCESS = """
             AND account.tenant_id = thread.tenant_id
             AND account.account_id = thread.account_id
             AND (
                 (account.account_kind = 'PERSONAL' AND account.owner_user_id = ?)
                     ...
                     AND EXISTS (
                         SELECT 1
                           ...
                           JOIN mail_shared_inbox_access_grants access_grant
                            ...
                            AND access_grant.member_state = 'ACTIVE'
                            AND access_grant.can_read = TRUE
                            AND access_grant.can_assign = TRUE
                            AND (access_grant.expires_at IS NULL
                                 OR access_grant.expires_at > CURRENT_TIMESTAMP)
                          WHERE policy.tenant_id = thread.tenant_id
                            AND policy.allow_shared_inboxes = TRUE
