-- Project the closed CHECK vocabularies introduced by Meeting V23/V24 into
...
CREATE TEMP TABLE tmp_v216_check_contract_manifest (
    ...
    CONSTRAINT ck_tmp_v216_contract_kind
        CHECK (contract_kind IN (
            'STATE_MACHINE', 'PROTOCOL', 'OBSERVABILITY')),
    CONSTRAINT ck_tmp_v216_values_nonempty
        CHECK (cardinality(allowed_values) > 0)
) ON COMMIT DROP;

INSERT INTO tmp_v216_check_contract_manifest VALUES
    ('MEETING.VM_MEETING_RECORDING_DELETION_COMMANDS.COMMAND_STATE',
     'vm_meeting_recording_deletion_commands.command_state',
     'STATE_MACHINE',
     ARRAY['FAILED', 'RUNNING', 'SUCCEEDED']::VARCHAR[]),
    ('MEETING.VM_MEETING_RECORDING_DELETION_HEALTH.HEALTH_KEY',
     'vm_meeting_recording_deletion_health.health_key',
     'OBSERVABILITY',
     ARRAY['RECORDING_RETENTION']::VARCHAR[]),
    ('MEETING.VM_MEETING_RECORDING_PROVIDER_COMMANDS.COMMAND_STATE',
     'vm_meeting_recording_provider_commands.command_state',
     'STATE_MACHINE',
     ARRAY['FAILED', 'RUNNING', 'SUCCEEDED']::VARCHAR[]),
    ('MEETING.VM_MEETING_RECORDING_PROVIDER_COMMANDS.COMMAND_TYPE',
     'vm_meeting_recording_provider_commands.command_type',
     'PROTOCOL',
     ARRAY['START', 'STOP']::VARCHAR[]);

DO $v216_preflight$
BEGIN
    IF EXISTS (
        SELECT 1
          FROM tmp_v216_check_contract_manifest manifest
        ...
        RAISE EXCEPTION
            'V216 found a conflicting active Meeting recording CHECK binding';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'V216 Meeting recording code-set key is owned by a different source';
    END IF;
END;
$v216_preflight$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'V216 Meeting recording CHECK contract convergence failed';
    END IF;
END;
$v216_postcondition$;
