-- Bind the Meeting recording Java enums introduced with V23/V24 to the
-- canonical CHECK-backed code sets projected by Platform V216. V216 is
-- immutable after deployment; this forward migration only adds typed bindings.
...
CREATE TEMP TABLE tmp_v217_typed_binding_manifest (
    ...
    CONSTRAINT ck_tmp_v217_contract_kind
        CHECK (contract_kind IN ('STATE_MACHINE', 'PROTOCOL')),
    CONSTRAINT ck_tmp_v217_values_nonempty
        CHECK (cardinality(allowed_values) > 0)
) ON COMMIT DROP;

INSERT INTO tmp_v217_typed_binding_manifest VALUES
...
BEGIN
    IF EXISTS (
        SELECT 1
          FROM tmp_v217_typed_binding_manifest manifest
        ...
        RAISE EXCEPTION
            'V217 canonical Meeting recording CHECK contract drifted';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'V217 found a conflicting Meeting recording typed binding';
    END IF;
END;
$v217_preflight$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'V217 Meeting recording typed binding convergence failed';
    END IF;
END;
$v217_postcondition$;
