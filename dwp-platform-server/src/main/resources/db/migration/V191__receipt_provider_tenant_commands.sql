CREATE TABLE sys_provider_tenant_command_receipts (
    ...
    CONSTRAINT uk_platform_provider_command_revision
        UNIQUE (provider_tenant_id, command_type, target_revision),
    CONSTRAINT ck_platform_provider_command_type
        CHECK (command_type IN ('LIFECYCLE', 'ENTITLEMENTS')),
    CONSTRAINT ck_platform_provider_command_revision
        CHECK (expected_revision >= 0 AND target_revision = expected_revision + 1),
    CONSTRAINT ck_platform_provider_command_hash
        CHECK (payload_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_platform_provider_command_result CHECK (jsonb_typeof(result_payload) = 'object')
);

CREATE INDEX idx_platform_provider_command_tenant
...
COMMENT ON TABLE sys_provider_tenant_command_receipts IS
    'Atomic provider command receipt. The platform mutation and receipt commit in the same transaction.';
