...
@Component
public class MeetingRecordingArtifactAssertionVerifier {

    private static final String PREFIX = "dwpraf1";
    ...
    @Autowired
    public MeetingRecordingArtifactAssertionVerifier(
            @Value("${dwp.meeting.recording-finalization.service-token:}") String token,
            @Value("${dwp.meeting.recording-finalization.assertion-key-id:}") String keyId,
            @Value("${dwp.meeting.recording-finalization.assertion-secret-base64:}")
            String secretBase64,
            ObjectMapper mapper) {
        this(token, keyId, secretBase64, mapper, Clock.systemUTC());
    }

    MeetingRecordingArtifactAssertionVerifier(
            String token,
            String keyId,
            String secretBase64,
            ObjectMapper mapper,
    ...
    VerifiedAssertion verify(
        ...
        try {
            ...
            if (payload.v() != 1 || !keyId.equals(payload.kid())
                    ...
                    || !recordingSessionId.equals(payload.recordingSessionId())
    ...
    private BaseException denied() {
        return new BaseException(
                ...
                "Trusted recording artifact producer identity is required.");
    }
